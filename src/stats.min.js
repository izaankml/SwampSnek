...
var Stats = function() {
  ...
  for (d.appendChild(e); 74 > e.children.length;) j = document.createElement(
      "span"), j.style.cssText =
  ...
  return {
    ...
    begin: function() { l = Date.now() },
      ...
      r++;
      b > m + 1E3 && (h = Math.round(1E3 * r / (b - m)), p = Math.min(p, h),
        q = Math.max(q, h), i.textContent = h + " FPS (" + p + "-" + q +
        ")", a = Math.min(30, 30 - 30 * (h / 100)), c.appendChild(c.firstChild)
        .style.height =
        a + "px", m = b, r = 0);
      return b
    },
